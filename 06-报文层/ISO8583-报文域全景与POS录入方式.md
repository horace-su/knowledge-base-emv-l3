...
- **MTI（Message Type Indicator）**：4 位。常见：`0100` 授权请求 / `0110` 授权响应 / `0200` 账务（消费）请求 / `0210` 响应 / `0400` 冲正请求 / `0420` 反转。本文用例截图里的 `0100/0200/1200` 即覆盖授权与账务两类，`1xxx` 是 ISO 8583:2003 版 MTI。`04xx` 冲正家族的报文要求、触发条件与 DE90/DE95 见 [冲正报文要求与触发条件](./ISO8583-冲正报文要求与触发条件.md)。
...
>
> 📑 三种受理路径（磁条 / 接触 / 非接）的报文域差异**整篇对照**（含 DE23↔`5F34` 卡序列号专节）见 [ISO 8583 报文差异对照：磁条 / 接触芯片 / 非接芯片](./ISO8583-磁条-接触-非接报文差异对照.md)。
