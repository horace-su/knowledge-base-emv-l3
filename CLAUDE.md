...
- **`web-docs/` holds the raw source PDFs** pulled during sessions; `web-docs/SOURCES.md` is the source list. Content lives in the tracked `.md` files; the PDFs are archival references. Extra unindexed source PDFs (EMVCo specs, L3 test reports, scheme rules) sit in `temp/`.
- **`pdftotext` is available** (`/usr/local/bin/pdftotext`, Poppler) and extracts clean text from the text-based PDFs — use it to fold PDF content into docs. Some PDFs are image-only (e.g. the Worldpay key tables), where only headers extract; for those, read the PDF directly via the Read tool. (Older docs/README still carry a stale "no PDF-extraction tools" note from before the tool was installed.)
...
- `emv-contactless-kernel-deep-dive.md` is currently untracked (not yet committed); the other docs are all committed.
