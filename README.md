...
- [DE55 标签分组速查（必填 / 条件 / 专有 / 响应侧）](./06-报文层/ISO8583-DE55-标签分组速查.md) —— 按**方向与必选度**重排的速查表（联机请求必填 / CDOL1·ARQC 输入 / 终端上报 / 条件必填 / 卡组织专有(AID-aware) / PCI 敏感 / 响应侧）+ 对"必填/可选"六处**常见误判勘误**（9F33 非 ARQC 输入 / 请求-响应方向混排 / 9F6E 复用 / 5A·57 慎入 55 / DF31 专有等价 9F5B / 9F34 条件必填）【速查·勘误篇】
...
- [ISO 8583 冲正（Reversal）报文要求与触发条件](./06-报文层/ISO8583-冲正报文要求与触发条件.md) —— 联机超时/异常后的 `0400`/`0420` 冲正：**MTI 家族**(请求/通知/重发) + **触发条件**(超时/无法完成/格式错/部分批准/卡过早移除) + **EMV 侧第二次 GENERATE AC=AAC 与外层冲正衔接** + **关键域**(DE90 原始数据元 / DE95 部分冲正金额 / STAN·RRN 匹配键 / DE39 原因) + DE55 是否随冲正上送 + 重发·幂等(`79` Already reversed)·必达 + 各卡组织差异 + **L3 怎么造超时验冲正**(`emvsim.*` 注入)与失败点速查 + **两份实现级逐域样本**：银联系收单主机(Cardnow V2.13 §7.11 `0400/0410`，原交易靠 **DE60.3/60.4** 而非 DE90、QR `Q8` 走查询不冲正) 、Mastercard 双消息(`0400/0420` + **DE30/DE90/DE95** 全额/部分冲正)、Visa BASE I(**Field 90/95** + `76`/`79` + 冲正归 Acquirer Host 测试)、Amex GCAG ISO(**实测:支持冲正、不支持退款**) 四体系同事异构落域总表 + **L3 范围问答**(加部分批准/冲正属非 EMV 处理、不重测)【冲正篇】
...
- 核查：全库 **40 篇内容文档**（报文层 11 篇）均被 README 索引且已进 nav/sidebar；`vitepress build` **零死链通过**。

| 检查项 | 结果 |
|--------|------|
| 内部链接（含新增冲正篇的全部 .md 交叉链 + 新增双向互链） | ✅ 全部有效，零失效 |
| README 文档覆盖 | ✅ 40 篇内容文档全部索引，无孤儿 |
